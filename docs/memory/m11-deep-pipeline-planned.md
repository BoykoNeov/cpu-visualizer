...
description: 'M11 (the 7-stage deep pipeline) - COMPLETE, all steps 0-8 done 2026-07-27; this file holds the PLAN and ENGINE half (steps 0-5 plus every pinned decision): the `IF1 IF2 ID EX1 EX2 MEM WB` split, EX2 as a real half-ALU, why the plan leads with the NET rather than INV-8, the hand-derived per-hazard coefficients, the mutation check, and the step-5 `models.ts` row that made a live crash reachable. Steps 6-8 (cache, datapath, browser pass) are in [[m11-deep-pipeline-view-and-cache]].'
metadata:
  node_type: memory
  type: project
...
**Steps 6, 7 and 8 — the cache detour that found a family-wide correctness bug, the bespoke
7-stage datapath, and the closing `vite preview` pass — are in
[[m11-deep-pipeline-view-and-cache]]** (split out of this file 2026-07-28, when it was 36.7 KB).
