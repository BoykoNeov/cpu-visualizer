...
steps 0/0b/1 done; the guard now admits widths 1..4 and the audit found no arity-2 code at
all. Six models ship, each with a lesson track. Repo **4504 tests**, five gates green.
...
- [Future microarchitectures](future-microarchitectures.md) — depth is DELIVERED (M11);
  **WIDTH is the one open axis.** Carries a ⚠ CORRECTION: its claim that the pairing rules
  ...
  - [M13 width — in progress](m13-width-planned.md) — steps 0/0b/**1** done. The rules were
    ...
    exported. Both gating decisions pinned. **Read before touching `engine/superscalar` or the
    lane hues** — the lane tints are a SECOND validated channel, not `PHASE_COLORS`, and the two
    share no constraint.
