...
- ⚠ **THE SAME MISTAKE RECURRED WITHIN THIS STEP, immediately after it was written up.** The first
  draft of #7's pin asserted `LESSONS.find(l => l.program === 'sum-loop')?.id === 'sum-loop-tour'`
  — an IDENTITY, over the WHOLE LIBRARY, where the sentence claims a MACHINE, over the lessons the
  reader has already passed. A second single-cycle lesson on this loop ahead of the tour leaves the
  prose true and reddens the assertion. Rewritten to search the PREFIX of the order (folding the
  precedence claim into the same expression) and to assert existence + model, with the id kept last
  as a labelled canary. **The recurrence is the finding: two instances in one step, by someone
  holding the rule, neither caught by a green suite or by a perturbation run.** The check that
  catches the class is asking of every assertion — what does the SENTENCE claim, and what does this
  EXPRESSION claim? They diverge most often in SCOPE (whole library vs. the reader's path) and in
  KIND (identity vs. property).
...
`M:\claud_projects\temp\m16-step5\` — `s5-scoreboard-lessons.mjs` (**260 checks, 0 failures**,
`run4.log`), `dump.probe.test.ts` + `probe.config.ts` → `oracle.json`, `findings.md`, screenshots.
...
⚠ **TWO FALSE SENTENCES, ONE FILTERED ORACLE.** Lesson 1 step 3 was false at BOTH tiers.
`detailed` said "every one of the thirty-four rows in this run looks the same" — the panel's own
window note prints **"the last 10 of 41 fetched"** at that lesson's closing cursor and **"43
fetched"** at the end of the run (43 rows, 34 that issue, 9 squashed fetches printed as all-dashes
with a `· flushed` marker). `expert` said the four columns are consecutive "for every instruction
in the recording" — those nine are in the recording with no four columns at all. **The step-1 oracle
is why both survived**: `rows(board()).filter(r => r.issue !== null)` then `.toBe(34)` discards the
very rows that make them false. **Green because it asserted the filtered thing** — the M15 step-8
shape one milestone later, and a new species of the step-4 lesson. Every number in it is correct and
none is about either claim. The discriminator is one question: **does the assertion's SCOPE match
the SENTENCE's scope?** ("instructions that issued" vs "every row in the recording" — no amount of
arithmetic care closes that gap.)

⚠ **AND THE FIRST TWO DRAFTS OF THE FIX WERE FALSE THE SAME WAY** — the third and fourth time in
this milestone that a correction repeated the mistake it was written to fix. Draft one wrote "the
table ends up holding forty-one", the count at step 4's cursor rather than at the end (43); caught
by the new pin. Draft two dropped the total and wrote "the counter above the table **reads higher**
than that" — present tense, antecedent thirty-four — while the note **at this step's own anchored
cycle** reads **"5 fetched so far"**. And the pin repeated the shape it was written to catch:
`toBeGreaterThan` compared 43 to 34, the relation at the END OF THE RUN, where the sentence asserts
one at THE READER'S CURSOR. Only reading the sentence back against the oracle's own `windowNote` for
that step found it.

**One rule under all three drafts: THE ROW COUNT GROWS WITH THE CURSOR (5 at cycle 8, 41 at 71, 43
at 79).** A sentence comparing it to a fixed number is false at every cursor before the crossover,
and a pin whose comparison is taken at the end of the run cannot see that. **Before pinning any
number in prose, ask whether the thing it counts grows with the cursor; if it does, quote only the
part that does not move, and pin the claim WHERE THE READER IS STANDING** — here, by building the
view at the step's own anchored cycle and asserting the note says 5, which is BELOW the sentence's
thirty-four. The shipped prose makes no comparison at all.

**Six stubs, each red on the clause it targets.** Stub 1 is the finding stated as a measurement:
with the original sentence restored, **the old filtered assertion stays green**. ⚠ **Stub 6 exists
because stub 4 did not prove what it looked like it proved** — stub 4 reddened an earlier clause in
the same test, so the anti-comparison guard was never reached. The step-4 "vitest stops at the FIRST
failed assertion" trap, aimed this time at a STUB table rather than a perturbation table: **a stub
that reddens the right TEST has not shown that the right ASSERTION fires.**
...
⚠ **A RIG THAT ONLY RENDERS THE DECLARED TIER CANNOT SEE TWO THIRDS OF THE PROSE.** The tier sweep
was first written once per lesson, on step 1 — leaving twelve of the fifteen steps' two non-declared
tiers unread, which is exactly where the `expert` sentence above was hiding. Widened to all three
tiers on EVERY step: 45 renders, about thirty seconds.
...
hop is what proves it fires more than once); all three tiers render correctly on all fifteen steps; every
