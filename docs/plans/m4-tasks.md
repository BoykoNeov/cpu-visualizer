...
**Status: STEPS 0–4 DONE — the ENGINE IS COMPLETE AND THE TOGGLE IS IN THE BROWSER, 2026-07-16
(685 → 764 tests). Step 4 shipped the second toggle: the config seam M3 cut paid off with no
widening, and `sum-loop` 78 → 71 / `call-return` 17 → 18 are verified on the live scrub bar. Its
real work was elsewhere — the control has TWO positions for three names (the acceptance line was
wrong, M4's third self-correction), `positionsFor` had silently gone half-blind the moment step 1
flipped the capability, and `Lesson.config` was NOT forwarding-shaped after all: weakening it to
`Partial` shipped a browser defect and was reverted, because a lesson is a controlled experiment
and `config` names the controls. PENDING: steps 5–7. Previous status follows.**
