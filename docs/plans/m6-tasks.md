...
**Status: STEPS 0–1 DONE, 2026-07-18 (985 tests). The corpus straddler ships and the pure timing-shadow
model is proven against the real engine's address stream; the pipeline does not consult it yet (step 2).
...
| **The new structural machinery — how does a miss stall?**        | **A countdown persisted in the MEM latch/`micro`**, decremented each cycle, freezing IF/ID/EX until zero — the load-use freeze primitive (raise in MEM, read by EX/ID/IF later in the reverse walk) extended from one-shot/one-stage to multi-cycle/multi-stage. Scout the exact hold mechanism in step 1; if it needs new machinery beyond the countdown, that is this milestone's headline cost and belongs stated, not discovered. | **SCOUTED (step 1), grounded in `processor.ts`; answer at the foot of `cache.ts`.** The load-use stall is a ONE-SHOT boolean recomputed each cycle — it CANNOT express a multi-cycle hold. A miss needs a **new persistent primitive**: a `missCyclesRemaining` countdown in the ExMem latch/`micro`, decremented to zero. It REUSES the reverse-walk signal-propagation shape but FREEZES IF/ID/EX (bubbling WB) rather than inserting one bubble. The countdown is the milestone's genuinely-new cost; the freeze wiring is the old primitive extended. |
