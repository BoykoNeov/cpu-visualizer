...
**Status: COMPLETE — STEPS 0–7 DONE, 2026-07-18 (1337 tests). Step 7 shipped the CACHE TRACK: three
lessons in a new `The cache` track (`cache-spatial` → `cache-temporal` → `cache-conflict`), the order
fixed BEFORE authoring and reviewed as a sequence (M5's finding applied up front), pinned by name.
`cache-spatial` (array-sum, LARGE) teaches the line-fill — first touch misses, its three neighbors
hit; `cache-temporal` (array-sum-twice, LARGE) teaches reuse — pass two revisits arr[0] and HITS;
`cache-conflict` (array-sum-twice, SMALL) teaches capacity/conflict + the flip — a third block evicts
the first, pass two re-misses, and flipping to large makes the eviction vanish. Steps anchor to
`cache-access` events (hit/miss/evicted/addr), never cycle numbers (INV-6); since that event carries
NO `instr`, the oracles pin `addr`/`hit`/`evicted` directly (there is no pc to pin the way the hazard
oracles do). Two of the three lessons have SIZE-EXCLUSIVE steps — temporal's revisit-hit is alive only
under large, conflict's eviction+re-miss only under small — exactly `branch-bet`'s config-exclusive
shape, covered by the sweep's "fires in ≥1 position" licence with NO validator special case (all 12
pipeline positions green first run). **The step-5 identity trap was reconciled by CANONICALIZING a
declared geometry to its shipped constant at LOAD** (`canonicalCache` in `lessons.ts` + a pure
`cacheEquals` in `session.ts`), which KEEPS the shell's "cache is always one of three constants"
contract true everywhere rather than swapping every `===` for a deep compare — so `setCache`'s guard
and `CacheToggle`'s lit-detection are unchanged, and a shipped test pins that every declared cache is
`===` a constant (a future typo'd geometry reddens). **Two findings the tests could NOT catch:**
(1) **A store emits a `cache-access` too** — `array-sum`'s `sw a0, 0(total)` is a 6th access (a hit),
absent from `cache.test.ts`'s loads-only verdict list — so spatial's payoff counts LOADS (5 loads,
2 misses), not accesses (6). Caught by DUMPING the real `cache-access` stream per (program × config)
before pinning any `nth` — the pin-against-a-real-trace discipline that carried steps 5–6. (2) **A
re-miss on a FULL cache is badged `EVICT`, not `MISS`** — conflict's pass-2 arr[0] re-fetch also
evicts block 2 (the two blocks thrash), so the grid reads `EVICT` where the draft prose said only
"misses"; the browser caught the mismatch and the narration now owns the eviction and deepens into
thrashing. **The THIRD lesson-authoring step and the pattern held: the browser caught a narration
CLARITY mismatch, not a shipped bug** — all three lessons were browser-verified (reconcile lights the
right toggle for small AND large; the grid's MISS/HIT/EVICT/FILLING match the prose; payoffs read
a0=120/156 in the register panel). 1236 → 1337 (+101).**

**Prior status: STEPS 0–6 DONE, 2026-07-18 (1236 tests). Step 6 shipped the CACHE GRID VIEW — a state
...
- [x] **7. The cache track — a SEQUENCE fixed here, not discovered. DONE 2026-07-18 (1236 → 1337,
      +101).** Three lessons in a NEW `The cache` track (after `The machine`), in the pinned order
      **spatial → temporal → conflict/capacity+flip**, fixed before authoring and reviewed AS a
      sequence — M5's finding (a track shipped in the wrong order because authoring a lesson never
      reads the others, and incremental insertion cannot see a sequence) applied up front, and pinned
      by name in `lessons.test.ts` (`cache-spatial < cache-temporal < cache-conflict`, grounded in the
      prose: temporal presupposes the line-fill spatial introduces, conflict presupposes the reuse
      temporal establishes). All anchor to `cache-access` events, never cycle numbers (INV-6).
  - **`cache-spatial`** — "A line, not a word" (`array-sum`, LARGE, forwarding on): first touch of a
    block misses and drags in a whole 16-byte line; the next three loads HIT (spatial locality); arr[4]
    misses at the block boundary. Payoff a0 = 120, "five loads, two misses". Verdict stream (from the
    engine dump) `M,H,H,H,M` over loads + a store HIT to `total`.
  - **`cache-temporal`** — "Come back and it is still there" (`array-sum-twice`, LARGE): pass one
    compulsory-misses each of three lines; pass two revisits arr[0] and HITS — the whole second pass
    hits because the 4-line cache held the 3-line working set. Payoff a0 = 156. The revisit-hit step is
    SIZE-EXCLUSIVE (dead under small).
  - **`cache-conflict`** — "Too small to hold it all" (`array-sum-twice`, SMALL): block 2 evicts block
    0 late in pass one; pass two re-misses arr[0]; flip to large and the eviction is gone. Payoff a0 =
    156, "five misses small / three large", symmetric flip prose (name-the-sides, per the README rule).
    The eviction + re-miss steps are SIZE-EXCLUSIVE (dead under large).
    **The reconcile — canonicalize at LOAD, keep the identity contract true.** Took the caveat's first
    option: `canonicalCache` (`lessons.ts`) maps a declared JSON geometry back to its shipped
    `CACHE_SMALL`/`CACHE_LARGE` constant when `LESSONS` is built, using a new pure `cacheEquals`
    (`session.ts`, no engine import — mirrors conformance's). So the shell still only ever holds one
    of three constants: `Simulator.setCache`'s `===` guard and `CacheToggle`'s `===` lit-detection are
    UNCHANGED (their step-5 caveats rewritten to "reconciled at load"), the other option (a deep
    compare at every `===`) declined to keep the contract true rather than paid-for-per-comparison. A
    shipped test pins every declared cache is `===` a constant, so a future typo'd geometry (lighting
    no toggle position) reddens.
    **Acceptance MET with NO validator special case:** all 12 pipeline sweep positions green first run
    — the size-exclusive steps ride the same "fires in ≥1 position" licence `branch-bet` established,
    and the SIZE axis triples it exactly as prediction doubled forwarding's. Per-lesson ORACLES pin the
    anchored payloads (`addr`/`hit`/`evicted`) and the size-exclusivity (the revisit-hit dead-on-small,
    the eviction+re-miss dead-on-large) — `cache-access` carries no `instr`, so there is no pc to pin;
    the addr IS the identity.
    **Two findings the tests could not catch (both surfaced by discipline, not luck):**
    (1) **A STORE emits a `cache-access`** — `array-sum`'s `sw a0, 0(total)` is a 6th access (a hit,
    `total` sharing arr[4]'s line), absent from `cache.test.ts`'s loads-only verdict list. So spatial's
    payoff and oracle count LOADS (5 loads, 2 misses), not accesses (6). Caught by DUMPING the real
    `cache-access` event stream per (program × config) before pinning any `nth` — the exact "pin
    against a real trace dump" ritual that carried steps 5–6.
    (2) **A re-miss on a FULL cache is badged `EVICT`, not `MISS`** — the browser showed conflict's
    pass-2 arr[0] re-fetch also evicts block 2 (the two blocks thrash), so the grid reads `EVICT` where
    the draft said only "misses". The narration now owns the eviction and deepens into thrashing.
    **The third lesson-authoring step, and the pattern held: the browser caught a narration CLARITY
    mismatch, not a shipped bug.** All three lessons were driven in a real browser — the reconcile
    lights the right toggle for small AND large (the step-7 trap, closed), the grid's
    MISS/HIT/EVICT/FILLING match the prose, and the payoffs read a0 = 120 / 156 in the register panel.
...
- [x] Final register + memory state **equals** the golden reference for every corpus program under
      **every** (forwarding × prediction × cache) config (INV-8) — green by construction, because the
      ...
      state still passes this; timing is its net. (Step 3's full 18-config differential.)
...
- [x] `engine/pipeline` still has **zero** imports from `web`/`curriculum`; the cache is honored via
      `ProcessorConfig` only, with no new back door — cache contents reach the view through `micro`
      in the trace, not through an accessor (INV-2/INV-3). (Lint's boundary rules stay green; the
      web-layer `canonicalCache`/`cacheEquals` import the two READ-surface constants, not engine logic.)
- [x] Every lesson still anchors under every config it declares it honors, including the new cache
      ...
      cache-track lessons landed in step 7 and are green across all 12 positions on the first run — the
      size-exclusive steps ride the "fires in ≥1 position" licence, no special case needed.
...
| **I-cache?**                                                     | **No — D-cache only.** An I-cache adds a second `cache-access` stream that muddies the array-walk lesson. Lawful under INV-5 provided no surface claims fetch is cached.                                                                                                                                                                                                                                                                                  | **PINNED (shipped as seeded).** D-cache only. Exactly one `cache-access` site exists — `stageMem` — so fetch is never cached and no surface claims it is (INV-5 lawful: the fetch path omits the cache, it never contradicts it). An I-cache stays a later tier's change.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| **`config.seed`.**                                               | **Stays unused this milestone.** Direct-mapped makes no replacement choice, so nothing needs randomness. Pin random-replacement as the _only_ future caller — this closes §73's "seed any randomness" concern for M6 entirely.                                                                                                                                                                                                                            | **PINNED (shipped as seeded).** Unused for the whole milestone. Direct-mapped makes no replacement choice, so nothing in the cache needs randomness; `ProcessorConfig.seed` is documented as such in `packages/trace/src/processor.ts`. **Random replacement is pinned as its only future caller** — which closes INV-1’s "seed any randomness" concern for M6 entirely.                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **What does `cache-access.level` read at single level?**         | **`1`** (there is one level: L1). Pin the constant so a future L2 adds `2`, and the view never has to guess whether `0` means "L1" or "no level."                                                                                                                                                                                                                                                                                                         | **PINNED (shipped as seeded): `1`.** Emitted as the literal `level: 1` in `processor.ts` and asserted in `cache-stall.test.ts`. A future L2 adds `2`; the view never has to guess whether `0` means "L1" or "no level."                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| **Cache-track lesson order.**                                    | **Spatial → temporal → conflict/capacity+flip**, fixed in this plan and reviewed as a sequence — M5's "incremental insertion cannot see a sequence" finding applied before authoring, not after shipping backward. **PINNED (step 7):** `cache-spatial` → `cache-temporal` → `cache-conflict`, a new `The cache` track, pinned by name in `lessons.test.ts` and forced by the prose (temporal presupposes the line-fill; conflict presupposes the reuse). | **PINNED (step 7).** `cache-spatial` → `cache-temporal` → `cache-conflict` in a new `The cache` track — fixed in this plan _before_ authoring (M5’s "incremental insertion cannot see a sequence" finding applied forward), then pinned by name in `lessons.test.ts` and forced by the prose itself: temporal presupposes the line-fill, conflict presupposes the reuse.                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **Set-associativity + replacement policy.**                      | **Out of scope.** A later tier; it is what buys a replacement-policy lesson and the one use of `config.seed`. Adding it is a `CacheConfig` change and its own milestone.                                                                                                                                                                                                                                                                                  | **PINNED: out of scope, and it stayed out.** Nothing shipped depends on associativity — `numLines` is also the number of sets. It is what would buy a replacement-policy lesson and the one use of `config.seed`; adding it is a `CacheConfig` change and its own milestone.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| **Second cache level (L2).**                                     | **Out of scope.** The `level` field anticipates it; modeling a hierarchy is a later tier.                                                                                                                                                                                                                                                                                                                                                                 | **PINNED: out of scope, and it stayed out.** The `level: 1` constant anticipates it (see the row above) and `missPenalty` is a flat cost precisely because modeling a hierarchy is a later tier’s business.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| **Relationship to M2 step 5c.**                                  | **Independent; 5c stays deferred.** 5c is the multi-cycle model's ALUOut→PC path. M6 touches the _pipeline_'s MEM stage. Do not let step 2 quietly absorb it.                                                                                                                                                                                                                                                                                             | **PINNED: independent; 5c is STILL DEFERRED.** Step 2 did not absorb it — M6 touched only the pipeline’s MEM stage, and the multi-cycle model’s ALUOut→PC path remains untouched and open.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
