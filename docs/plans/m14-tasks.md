...
- [x] **4. Wire the track — and pin the order on the sentences that lie without it. ✅ DONE
      2026-07-30**, `16b596c`. Repo 7106 → **7107** (both totals measured — the baseline by checking
      out the parent's `lessons.test.ts` and re-running, not inferred from the last step's log); all
      five gates green; **7 breaks run, 7 gave the intended result** (6 reddened, 1 deliberately
      stayed green). **Every one of the five ordering claims was violated on its own**, not just the
      two the first pass happened to cover — an asserted link never broken is the shape this
      milestone has been catching all along. The wiring was already done —
      each lesson appended its own id as it landed, so `index.json` shows seven — and the diff
      confirms it stayed inside the track's `lessons` array: `git diff 458b4ce~1..HEAD` touches the
      by-name membership `toEqual` and nothing else in that block. **The track-name `toEqual` and
      both pairwise track-order checks are UNTOUCHED across the whole milestone** — measured on the
      hunk offsets, not inferred from them looking right now, because "extending touches neither" is
      a claim about what steps 1–3 did and the current file cannot settle it.
      What the step found, beyond what the plan predicted:
      **The discriminator applies per MENTION, not per track, and applying it rejects three
      references outright.** The seven lessons name each other eleven times; five of those mentions
      are pinned (the rest restate a link already pinned), and only mentions that are claims about
      what the reader has ALREADY done qualify. "Compare \"The pair that can't\",
      where the same event with the same reason string splits work…" (in both `where-widening-stops`
      and `four-in-a-row`) is an INVITATION — equally true whether or not that lesson has been read,
      and the deeper machine's own docblock already draws the line: _unexplained is not a lie, and a
      pin that fires on neither is a rule with nothing behind it._ Likewise `width-moved-the-work`
      step 1's expert "\"Four in a row\" goes 6, 13, 12" is a fact about another lesson's
      measurements, not about the reader's history — that link is pinned, but on step 2's `detailed`
      tier, which is past tense and does lie. So `pair-that-cant`'s position is **unpinned**, and
      `one-door` is named by nothing and names nothing and earns no pin at all.
      **The pinned chain is broader than the decision row's question.** Two of the four earned links
      reach back into the M8 four (`two-at-once` → `where-widening-stops`, `one-branch-unit` →
      `width-moved-the-work`), which a row about "the within-track order of the three" did not
      anticipate. The three new lessons are not orderable among themselves alone.
      ⚠ **`indexOf` makes an order pin VACUOUS under a rename, and that was measured rather than
      reasoned.** It returns `-1`, and `-1` is less than every real index, so every comparison passes.
      Renaming `two-at-once` in the index with the guard removed leaves this test **green** on an id
      that no longer exists. The `toBeGreaterThanOrEqual(0)` guard (the OoO pin's shape) is
      load-bearing for this test — though it is NOT the sole net for a rename: three other tests in
      the file catch one, so the comment says so rather than overclaiming.
      **The order pin IS the sole net for a reorder, which is the opposite result from step 1's.**
      Each of the five orderings, violated alone, reddens exactly ONE test out of 7107 — swap the two
      delta siblings, swap `where-widening-stops` with `four-in-a-row`, move `two-at-once` to the end,
      move `one-branch-unit` past `width-moved-the-work` — and so does deleting a referring sentence.
      Nothing else in the suite can see any of them.
      **The referring sentences were checked for TRUTH, not just for presence** (an advisor's catch:
      pinning that a sentence exists is not pinning that it is right, and making a false one
      load-bearing is worse than not pinning it). The suspect one was `four-in-a-row`'s ""Where
      widening stops paying" left you at four slots" — `where-widening-stops` asks for **three**. Its
      closing step then says "set the ISSUE control to 4 and watch the count refuse to move", so the
      successor's sentence is exact, down to the phrase.
      **The unpinned positions are a KNOWN, tested gap.** Shuffling the four M8 beats among
      themselves (`one-door` first, `pair-that-cant` last) leaves all 1926 tests in the file green —
      the membership pin sorts, and the order pin deliberately claims nothing there. Recorded in the
      shape `racing-ahead-of-the-miss`' cache-off blindness is recorded: a measured hole rather than
      a surprise.
      Acceptance met: the picker shows seven under "The wide machine"; the track-name and pairwise
      pins are untouched; every order claim is justified by a named sentence asserted beside it, at
      the TIER that sentence occupies (`width-moved-the-work`'s expert tier says "the other two
...
| Within-track order of the three                 | Thesis → flagship → conditional, appended after `one-branch-unit`. Pin a sequence test ONLY if a prose sentence lies when reordered (the cache track's discriminator)                                                                                                                                                              | **PINNED as seeded, and the pin is BROADER than this row's question** (step 4, 2026-07-30). The track reads `where-widening-stops` (5), `four-in-a-row` (6), `width-moved-the-work` (7), and the pinned chain is `two-at-once` < `where-widening-stops` < `four-in-a-row` < `width-moved-the-work` plus `one-branch-unit` < `width-moved-the-work` — two of the four earned links reach back into the M8 four, so the three are not orderable among themselves alone. The discriminator was applied per MENTION, not per track: of the eleven times these lessons name each other, five are pinned and three are rejected outright (both "Compare \"The pair that can't\"" invitations, and the expert-tier quotation of `four-in-a-row`'s refusal row — a fact about another lesson's numbers, not the reader's history), so `pair-that-cant` is unpinned and `one-door` earns nothing. Each surviving link asserts its sentence beside the index, at that sentence's own tier |
