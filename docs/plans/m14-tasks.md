...
authored, and step 4 pinned the track's within-track ORDER on the four cross-references that go false
when it changes (of seven; three were rejected as invitations rather than lies). **Only step 5
remains** — the browser pass. Original step-0 note follows.**
...
- [x] **4. Wire the track — and pin the order on the sentences that lie without it. ✅ DONE
      2026-07-30**, `16b596c`. Repo 7106 → **7107**; all five gates green; **4 breaks run, 4 gave
      the intended result** (3 reddened, 1 deliberately stayed green). The wiring was already done —
      ...
      What the step found, beyond what the plan predicted:
      **The discriminator applies per REFERENCE, not per track, and applying it rejects three of the
      seven cross-references.** The wide track's lessons name each other seven times; only four of
      those are claims about what the reader has ALREADY done. "Compare \"The pair that can't\",
      ...
      **The order pin IS the sole net for a reorder, which is the opposite result from step 1's.**
      Swapping `four-in-a-row` and `width-moved-the-work` reddens exactly ONE test out of 7107; so
      does deleting a referring sentence. Nothing else in the suite can see either.
...
- [x] "The wide machine" track shows seven lessons; the three new ones open on the superscalar at
      `issueWidth: 2` and at their declared depth tier. (Step 4 — pinned by the by-name membership
      `toEqual`, the library-wide `issueWidth`/`depthDefault` pins, and the order pin.)
