...
- **The renaming A/B** — 31 → 30 on the scoreboard, 13 → 13 on out-of-order, driven through
  ...
  `function-call`'s hand-off oracle, and it is the only pin that positively demonstrates _renaming_
  rather than merely _absence_.
...
- **Step 3 — the WAR lesson**, and the reorder at c17. The only WAR in the product; the only stall
  ...
  answer. Carries the `reservation-station-holds` callback from finding 3, and — if decision 6 says
  so — the track's hand-off, the rename edit from finding 1.
...
- Three lessons (or the number decision 1 pins) authored, each listed in `index.json`, each swept
...
| --- | ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- |
| 1   | **How many lessons, and which subjects**       | **Three: the ceiling, WAW, WAR+reorder.** Matches M12 and M14. A fourth ("finished out of order" as its own lesson) is available but its only cycle is c17, which lesson 3 needs; splitting it costs lesson 3 its payoff. Gates steps 1–3                                                                                                                                                         | _open_        |
| 2   | **The ceiling lesson's program**               | **`add.s`** — three instructions, 9 cycles, no hazard at all, and the reader met it as `first-program`. ⚠ `deep-drain` already teaches `add.s` as "Three instructions, ten cycles" on the deep pipeline, so the two are siblings and their titles must not collide. Alternative: **`sum-loop`** (80 cycles, IPC 0.425, zero data-hazard stalls) — realistic scale, weaker punch                   | _open_        |
| 3   | **A new track vs extending an existing one**   | **New track**, appended last. A new model is M12's case, not M14's; extending "The out-of-order machine" would file a non-renaming machine under a renaming one. Costs a hard edit to the exhaustive track-NAME `toEqual` and to the picker order, both expected. Name seeded as **"The scoreboard"**                                                                                             | _open_        |
| 4   | **Track position**                             | **Last, after "The out-of-order machine"** — and the pin is EARNED rather than assumed, by finding 3: lesson 3's callback ("there is nowhere to park, so it waits in the unit") is not merely unexplained but false in its premise if the reader has not met the reservation station. Historically the scoreboard PRECEDES Tomasulo; M15's own rule appends a predecessor met after its successor | _open_        |
| 5   | **Does the track close with a HAND-OFF**       | **Yes** — the rename edit from finding 1, on the track's LAST step, and only there (running an edit forks to the sandbox and DETACHES the lesson, so the narration vanishes the instant it is obeyed). It is the only pin that positively shows renaming. ⚠ The picker's last track is where "go edit" is lawful; if decision 4 moves, this moves with it                                         | _open_        |
