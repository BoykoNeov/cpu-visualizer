...
**Status: STEPS 0–4 DONE, 2026-07-16 (440 → 554 tests). The pipeline model exists, is INV-8-clean
...
INV-8 structurally cannot see — and it is time-travellable, with five instructions in flight and
individually followable (step 4).** Proven so far: that the seams M3 fills already existed (`ProcessorConfig.forwarding`,
...
- [x] Load → step forward to completion → step **backward** to start → **scrub** to any cycle;
      shown state always matches the recorded trace. ✅ step 4, headlessly. The **live scrub bar**
      half is step 5's.
