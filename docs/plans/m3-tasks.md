...
**Status: STEPS 0, 1 and 2 DONE, 2026-07-16 (440 → 500 tests). The pipeline model exists, is
INV-8-clean under both forwarding positions, and its soul is pinned by 31 hand-derived unit
...
intra-cycle ordering decision was added. Building it then forced **eleven more** (the trace
...
- [x] **1. `engine/pipeline` — the model MVP.** ✅ Done (2026-07-16, 457 → 500 tests).
      `@cpu-viz/engine-pipeline` implements `Processor` in ~600 lines of `processor.ts`, wired into
      the DAG in all four places plus the npm `workspaces` list (a fifth the plan forgot). 31
...
Rows still `_(open)_` are the three view-layer ones (datapath tiering, map tech, follow-highlight),
which belong to steps 6–7.
...
| `flush.stages` semantics           | _(not in the seed)_                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | **Pinned**, 2026-07-16: it names the latches the flush SIGNAL is asserted on (`['IF','ID']` for a taken branch, `['IF']` for a halt), **not an inventory of who was standing there** — the hardware asserts it either way. Views cross-reference `instructions[]` for the casualties. Keeps step 3's assertion stable regardless of occupancy.                                                                                                     |
