...
- [ ] **1. `engine/pipeline` — the model MVP.** New workspace package `@cpu-viz/engine-pipeline`
  ...
  - **Control hazard**: resolve in EX, fixed predict-not-taken; a taken branch emits
    ...
    too** — its target is register-dependent, which makes it the one control instruction that is
    _also_ a RAW consumer, so it is precisely where the hazard unit and the flush logic interact.
    `call-return.s`'s `ret` (= `jalr x0, 0(ra)`) exercises exactly this; do not discover it in
    step 2.
  - **Halt with drain — the correctness trap.** This is new, it is not cosmetic, and the corpus
    ...
    - **`ecall`** (`sum-loop.s`, `array-sum.s`, `byte-loads.s`, `call-return.s`). The reference
      ...
      and must be squashed — the stages behind it hold _older_ instructions. And the shadow is
      not necessarily garbage: in `call-return.s` the `ecall` at line 16 is followed by the real
      `max:` function (`bge`, `mv a0, a1`, `ret`). That shadow holds a branch and an indirect
      jump, so "younger instructions never commit" is **not sufficient** — they must not
      **redirect the PC** either.
...
| Branch resolution stage            | **EX** — a 2-cycle flush, no ID comparator, and more visually dramatic than resolving early. Emit `branch-resolved` + `flush`.                                                                                                                                                                                                                                                                                                                                                                                                                     | **Pinned as seeded, extended: `jalr` resolves in EX too** — register-dependent target, so it is the one control instruction that is also a RAW consumer (`call-return.s`'s `ret`). See step 1.                                                                                                       |
...
| Halt semantics                     | **Architectural `pc` is the retiring instruction's `nextPc` — never the fetch pointer**, copying multi-cycle's retire arm. Fetching stops on `ecall`-in-ID _or_ the fetch pointer leaving `.text`; either way, drain and halt at the last retire. Younger instructions neither commit **nor redirect the PC**. _(The original seed — "stop fetching at `ecall` decode, drain, halt at its retire" — was **verified false about the corpus**: `add.s` has no `ecall` and halts by running off the end of text, an entirely unhandled second path.)_ | **Pinned as rewritten**, 2026-07-16 — see step 1 for the corpus evidence and why `expectEquivalent`'s `pc` assertion makes this load-bearing.                                                                                                                                                        |
