...
**Status: STEPS 0–5 DONE, 2026-07-16 (440 → 587 tests). The pipeline model exists, is INV-8-clean
...
individually followable (step 4), and it now RUNS IN THE BROWSER with the forwarding toggle
shipped (step 5): 78 cycles off → 56 on, `a0` = 55 in both, on the live scrub bar.** Proven so far: that the seams M3 fills already existed (`ProcessorConfig.forwarding`,
