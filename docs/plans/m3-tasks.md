...
**Status: COMPLETE — ALL STEPS 0–8 DONE, 2026-07-16 (440 → 685 tests). The pipeline model exists, is
INV-8-clean under both forwarding positions, its soul is pinned by 32 hand-derived unit tests, its
TIMING is pinned on the whole corpus by a closed-form derivation (step 3) — the net for the one thing
...
network vanishes when the toggle flips — it has the PIPELINE MAP (step 7), the textbook
stage×cycle grid, where the overlap finally stops being a claim about the trace and becomes the
staircase everyone recognizes — and it now has THE FLAGSHIP LESSON (step 8), where the spec's
experiment stops being something the UI merely permits and becomes something the product
teaches.**

> **What step 8 says about the whole milestone.** The lesson needed **no new lesson-format field, no
> engine change, and no renderer change** — it is a JSON file. Its steps are config-exclusive by
> construction (`'raw'` fires only with forwarding off; `'load-use'` and `forward` only with it on),
> so flipping the toggle mid-lesson makes its middle two beats **swap**, and that worked for free:
> `narrationView` already dropped never-fired steps and `activeStepAt` already skipped null anchors.
> Everything M3 built to be oblivious turned out to be oblivious. The one thing that had to change
> was the seam nobody had noticed — `startLesson` ignored `lesson.model` and `lesson.config`, both
> declared and honored by nobody since M1, the same shape `ProcessorConfig.forwarding` had before
...
- [x] **8. The flagship lesson — "watch the bubble vanish."** ✅ Done (2026-07-16, 658 → 685 tests).
      `content/lessons/forwarding-bubble.json` — six authored steps on `array-sum`, **zero new
      lesson-format fields**, **zero engine changes**. Browser-verified end to end: picking the
      lesson swings the shell from single-cycle to **Pipeline**, the scrub bar to **72 cycles**, the
      forwarding toggle and the map appear, and the rail reads **4 steps** — six authored, two
      dropped as belonging to the other position.

      **The step's one real idea: the lesson's steps are CONFIG-EXCLUSIVE, and that is the lesson.**
      The pinned event vocabulary settles it before any authoring: `stall.reason: 'raw'` fires only
      with forwarding OFF, `'load-use'` and `forward` only with it ON. So a lesson about a stall that
      disappears when you flip a toggle is a lesson **some of whose steps must be dead in one
      position** — there is no honest authoring of the flagship experiment in which every step fires
      in both. That reads like a format gap and is not one: `narrationView` already drops never-fired
      steps and `activeStepAt` already skips null anchors, so the flip works **for free**. Flipping
      the toggle re-records, re-anchors, and the rail's middle two beats **swap**. The runtime was
      done before the step started; only the VALIDATOR was broken.

      > A `LessonStep.requires` field was designed and then **rejected before it was written**. It
      > would have been M3's third speculative field, after step 6 dropped `maxTier` and step 7 found
      > delta 2 obsolete. `model` + `config` already declare everything it would have said.

      **The program was forced, not chosen.** `array-sum` is the only corpus program whose load-use
      stall survives forwarding (step 3), and it carries **both** halves on source-visible lines:

      | source line              | forwarding off | forwarding on                   | teaches           |
      | ------------------------ | -------------- | ------------------------------- | ----------------- |
      | `add a0, a0, t2` (pc 20) | stall `raw` ×2 | stall **`load-use`** ×1 → 5 tot. | the bubble that _cannot_ vanish |
      | `bnez t1, loop` (pc 32)  | stall `raw` ×2 | **forward**, 0 stalls           | the bubble that vanishes |

      Two hazards, two source lines, opposite fates, one program. The `add` does not lose its bubble
      — it **shrinks**, 10 stall cycles to 5, and is renamed. That contrast is the whole lesson, and
      it is why the beats are ordered off/off then on/on: each config reads its own subsequence in
      order, which holds in **both** (off: 0 → 8 → 13 → 68; on: 0 → 6 → 11 → 47).
  - **The seam the plan did not mention, for the fourth view step running: `startLesson` ignored
    `lesson.model` and `lesson.config` entirely.** Both were declared-and-honored-by-nobody since M1
    — the same shape as `ProcessorConfig.forwarding` before this milestone. Invisible while every
    lesson targeted the default model and had no config opinion; wrong the moment a lesson is _about_
    a microarchitecture. Extracted as **`lessonOpening`** (pure, in `session.ts`, per the
    `narration.ts`/`session.ts` precedent — a real decision does not belong inline in a hook). The
    asymmetry is the finding: **`model` is always honored; `config` only when DECLARED**, because the
    position is session-level and persists (step 5), so `undefined` means "no opinion", NOT "fall
    back to the default" — and `defaultConfig().forwarding` is `false`, so a naive fallback looks
    right in the common case and silently clobbers exactly the user who turned it on.
  - **Honoring `model` fixes a bug that outlived M2, and reframes what the cross-model suite proves.**
    A lesson's ANCHORS survive a model swap — that is INV-6 and `lessons.test.ts` has pinned it since
    M2. Its NARRATION does not: `sum-loop-tour` tells the reader its add is "written back to a0 in
    the same cycle", which is true on single-cycle and **false on both other models**. No anchoring
    test can see that, because **anchoring is not truth**. The cross-model suite proves portability of
    the anchors and was silently read as portability of the lesson.
  - **The validator was SCOPED, not weakened — and derived rather than declared.** One rule replaces
    the hardcoded single-cycle drive: _run each lesson under the model it declares, across every
    config position that model **honors** (`capabilities.configurableForwarding` — the same gate the
    shell uses for the toggle, so the suite and the UI cannot disagree); every step anchors in **at
    least one**; order and the shared-cycle guard are checked **per recording**._ It degenerates
    correctly with no special case: a config-blind model has one position, where "at least one" IS
    the old strict "no dead steps"; the pipeline gets two, where a config-exclusive step is lawful and
    a typo — dead in **both** — still fails.
  - **What that rule structurally cannot see is the pedagogy, so it is asserted positively and by
    PC.** "Fires in at least one position" is satisfied just as well by a step pointing at the wrong
    hazard: `array-sum` stalls at three distinct pcs. So the oracle pins each beat to the pc it stalls
    at, resolved through the recording's own `instr-fetch` events (ids are minted per fetch and are
    meaningless across two recordings; the pc is what says "that source line"). **That is what makes
    the `nth` counts reviewable** — `nth: 3` becomes a claim about _which hazard_, not a number read
    off a run. Mutation-checked, and the mutation is the one a reasonable author would hit first:
    weakening it to `nth: 1` slides the step onto the **`la` pseudo-op's hidden internal RAW** (the
    trigger a first draft would reach for, since `la` looks like one instruction and expands to two).
    The entire generic sweep stays **green** — a real `raw` stall, alive off, dead on, in order,
    narrated — and exactly one assertion fails: `expected 4 to be 20`.
  - **And for the FIFTH step running, the browser eyeball caught what no test did.** Every headless
    net was green while the flagship lesson rendered `**not**` to the reader as four literal
    asterisks. The cause is structural, not careless: narration is **plain text with exactly one
    markup construct** — the backtick code span `renderNarration` splits on. Nothing says so; the
    format is defined entirely by what the renderer happens to split on, and an author with ordinary
    Markdown reflexes gets no signal. The deeper gap is that **every test asserted narration
    RESOLVES — that a string comes back at the tier — and none asserted it RENDERS.** Those are
    different claims and only the first was ever tested.
    Fixed by **subtraction, not by teaching the renderer Markdown**: three lessons prove the plain
    vocabulary is sufficient, so the fourth was the outlier in _style_, not in need. Growing shared
    infrastructure for one lesson would have split the library into two formatting tiers — which
    reads as inconsistency, not richness — and the rewrite is simply better prose: "the bubble that
    does **not** vanish" became **"Here is the stall that survives forwarding"**, which carries the
    negation in the sentence. _Structural emphasis beats bold; load-bearing meaning should not rest
    on `<strong>`._ The durable deliverable is the guard: strip the one supported construct, assert
    no `*` survives. Deliberately narrow — asterisks are the hard failure (punctuation on screen),
    while a stray `\n` merely collapses to a space, and forbidding it would be a style rule wearing a
    test's clothes. Pin what breaks, not what one author would have done differently.

    _Original plan text:_ The experiment the spec names, made
    ...
    **Correction to this step's own acceptance line, made before authoring.** "Anchors … in both
    configs" cannot mean _every step fires in both_ — the same plan pins that `'load-use'` fires only
    with forwarding on and `'raw'` only with it off, so the line would forbid the lesson it asks for.
    Read per-config, as it was meant: **in each position, the steps that apply anchor in order with
    resolvable narration.** A step lawfully dead in one position is the point, not a violation.
...
- [x] `engine/pipeline` has **zero imports** from `web`/`curriculum` and from any other engine's
      production code; the trace schema is the only shared type surface (INV-2/INV-3, mechanically
      enforced). ✅ true since step 1 and never at risk, because it is not a thing anyone remembers
      to check: `eslint.config.js` fails the build on a cross-layer import and cites the invariant in
      the message. Worth checking off rather than deleting — the milestone's whole shape (a lesson
      that changes what it teaches when a config flips, read by views that were never told a pipeline
      exists) rests on it, and step 8 is where that stopped being a claim: the flagship lesson is
      **data**, and the surfaces that play it (`narrationView`'s drop of dead steps, the map's
      `hasOverlap` gate, the transport's `instructions.length > 1` rule) each do the right thing for
      it while containing no model knowledge at all.
- [x] **The flagship experiment is GUIDED, not merely possible** (§12.2). ✅ step 8:
      `forwarding-bubble` stops at the interlock with forwarding off, and — on the same two source
      lines — at the load-use stall that survives and the branch stall that vanished with it on.
      One lesson, both positions, its middle beats swapping as the user flips the toggle underneath
      it. That is INV-6's reason to exist, spent on the thing the milestone was for.
...
**Step 8 added four more, and closed the milestone.** The pattern held to the end, and sharpened
once: what building forced was again _contract_ rather than architecture — but this time the contract
that broke was one **nobody had ever written down**. The lesson narration format is "plain text plus
backtick code spans", a rule that exists nowhere except in the four lines of `renderNarration` that
split on backticks; the flagship lesson reached for `**bold**` and shipped literal asterisks to the
reader. And the step's headline decision was a **deletion**: a `LessonStep.requires` field was
designed and rejected before it was written, because `model` + `config` already said it — the third
time this milestone declined a field (step 6 dropped `maxTier`, step 7 found delta 2 obsolete).
For the **fifth step running**, the browser eyeball found what no test did.
...
| A lesson step may be lawfully DEAD in a config | _(not in the seed — every prior lesson anchored to purely architectural events, which every model emits)_                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | **Pinned**, 2026-07-16 (step 8): **yes, and no new field says so.** The pinned vocabulary makes it unavoidable — `stall.reason: 'raw'` fires only with forwarding OFF, `'load-use'`/`forward` only with it ON — so a lesson about a vanishing stall MUST have steps that die in one position. A `LessonStep.requires` predicate was designed and **rejected before being written**: `narrationView` already drops never-fired steps and `activeStepAt` already skips null anchors, so the flip works free at runtime; only the VALIDATOR was broken, and `model` + `config` already declare what `requires` would have. Third field this milestone declined.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| How a lesson's steps are validated             | _(not in the seed — it said "the lesson anchors in order … in both configs" and stopped there)_                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | **Pinned**, 2026-07-16 (step 8), and the acceptance line itself was **corrected** first: read literally it forbids the lesson it asks for. The rule is DERIVED: _drive each lesson under its DECLARED model, across every config position that model **honors** (`capabilities.configurableForwarding` — the shell's own toggle gate, so suite and UI cannot disagree); every step anchors in **at least one**; order + the shared-cycle guard **per recording**._ Degenerates with no special case — one position ⇒ "at least one" IS the old strict rule; two ⇒ config-exclusivity is lawful while a typo (dead in BOTH) still fails. What it structurally cannot see is WHICH hazard a step points at, so the pedagogy is asserted separately, **by pc** (resolved via `instr-fetch`; ids are per-fetch and meaningless across recordings). That is what makes `nth` reviewable. Mutation: `nth: 3`→`1` slides onto the `la` pseudo-op — sweep green, one oracle fails `expected 4 to be 20`.                                                                                                                                                                                    |
| What starting a lesson does to model + config  | _(not in the seed — `Lesson.model`/`Lesson.config` had been declared and ignored since M1)_                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | **Pinned**, 2026-07-16 (step 8) — step 8's unmentioned seam, the fourth view step running to have one. `startLesson` loaded a lesson's program under whatever the picker was set to. Pinned as **`lessonOpening`** (pure, in `session.ts` — a real decision does not belong inline in a hook), and the ASYMMETRY is the answer: **`model` always honored** (a lesson is prose about ONE machine — `sum-loop-tour` says its add is "written back to a0 in the same cycle", false on both other models; **anchoring is not truth**, and the cross-model suite proves the ANCHORS port, not the words); **`config` honored only when DECLARED**, since the position is session-level and persists (step 5), so `undefined` = "no opinion", not "use the default" — and `defaultConfig().forwarding` is `false`, so a naive fallback would look right in the common case and clobber precisely the user who had turned it on. Opening position only: picker + toggle stay live.                                                                                                                                                                                                         |
| The lesson narration VOCABULARY                | _(not in the seed — nothing had ever documented one)_                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | **Pinned**, 2026-07-16 (step 8) — **found by the browser eyeball; every headless net was green.** Narration is **plain text with exactly ONE construct: the backtick code span**. That rule lived nowhere but in the four lines of `renderNarration` that split on backticks, so `forwarding-bubble`'s `**not**` reached the reader as four literal asterisks. The structural gap: every test asserted narration **RESOLVES** (a string comes back at the tier); none asserted it **RENDERS**. Fixed by **subtraction, not by teaching the renderer Markdown** — three lessons prove the vocabulary sufficient, so the fourth was the outlier in style, not need; enriching the renderer for one lesson would split the library into two formatting tiers. The rewrite is better prose anyway ("Here is the stall that survives forwarding" carries the negation structurally). Guarded narrowly: strip code spans, forbid `*`. A stray `\n` merely collapses to a space, so it is NOT flagged — pin what breaks, not what one author would have done differently.                                                                                                                  |
